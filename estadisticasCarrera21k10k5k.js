...
// Obtener la tabla y añadir un listener para escuchar los clicks en sus filas
const tablaMedallero = document.getElementById("tabla");
tablaMedallero.addEventListener("click", mostrarCompetidoresTeam);

// Función para mostrar los competidores de un team en una ventana emergente
function mostrarCompetidoresTeam(e) {
  // Si el click fue en una fila de la tabla
  if (e.target.tagName === "TD") {
    // Obtener el nombre del team de la fila en la que se hizo click
    const nombreTeam = e.target.parentNode.querySelector("td:nth-child(2)").textContent;

    // Filtrar el array de competidores para obtener los que pertenecen al team seleccionado
    const competidoresTeam = competidores.filter((competidor) => competidor.runningTeam === nombreTeam);

    // Crear el contenido de la ventana emergente con los nombres y edades de los competidores del team seleccionado
    let contenido = "<h2>Competidores del Team " + nombreTeam + "</h2>";
    contenido += "<ul>";
    competidoresTeam.forEach((competidor) => {
      contenido += "<li>" + competidor.nombre + " (" + competidor.edad + " años)</li>";
    });
    contenido += "</ul>";

    // Mostrar la ventana emergente con el contenido creado
    alertify.alert(contenido);
  }
}
