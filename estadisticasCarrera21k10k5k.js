...
medalleroTeam.addEventListener('click', (e) => {
    if (e.target.tagName === 'TD') {
      // Obtener el nombre del running team de la fila
      const runningTeam = e.target.parentNode.children[1].textContent;
  
      // Filtrar los datos de los competidores por running team
      const competidoresGanadores = competidoresTeam.filter(competidor => competidor.runningTeam === runningTeam);
  
      // Crear la ventana emergente con los datos de los competidores
      const ventanaEmergente = window.open("", "competidoresGanadores", "width=400,height=400");
      ventanaEmergente.document.write('<h1>Competidores ganadores del running team ' + runningTeam + '</h1>');
      competidoresGanadores.forEach(competidor => {
        ventanaEmergente.document.write(`<p>Nombre: ${competidor.nombre} ${competidor.apellido} - Edad: ${competidor.edad} - Posición por edad: ${competidor.posicionEdad} - Posición por sexo: ${competidor.posicionSexo} - Posición general: ${competidor.posicionGeneral}</p>`);
      });
    }
  });
